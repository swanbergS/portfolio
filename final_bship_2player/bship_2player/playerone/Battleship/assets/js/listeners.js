...
document.querySelector("#gametable tbody").addEventListener('click', e => {
	...
	if((tableElement !== null) && (tableElement.tagName.toLowerCase() == "td")) {
		...
		$.get("http://127.0.0.1:3000/player1", {status: stat, xcoord: x-1, ycoord: y-1, username: p1name, pcount: playcount}, function(result){
            //console.log(result);
			/**This will grab the mini table and update it based on the array sent back*/
			for(let i=0; i < result.length; i++){
				for(let j = 0; j < result[i].length; j++){
					let tableElement = document.getElementById("p2table").children[0].children[i+1].children[j+1];
...
document.getElementById("reset").addEventListener("click", function(){
	game.reset();
	$.get("http://127.0.0.1:3000/player1", {status: "reset", xcoord: 0, ycoord: 0, username: p1name}, function(result){
			console.log(result);
	});
	...
	if(playcount > 1){
		$.get("http://127.0.0.1:3000/getscore", {username: p1name, player: 1}, function(result){
			let format = JSON.parse(result);
			dbscore = format.score; //database score
			//console.log(dbscore);
			$.get("http://127.0.0.1:3000/changescore", {username: p1name, score: dbscore}, function(result){
				console.log(result);
			});
		});
	}
